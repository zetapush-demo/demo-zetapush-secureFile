...
import { Anchor, WoleetApi } from '../woleet/woleet-api.service';
...
constructor(private api: FileApi, private client: ZetaPushClient, private sanitizer: DomSanitizer, private woleetApi: WoleetApi) { }
...
confirm(request: FileUploadRequest, metadata: any): Promise<FileUploadRequest> {
    ...
    shaObject.update(result);
    let hash = shaObject.getHash("HEX");

    // Create a new Anchor
    this.createAnchor(result['name'], hash);
